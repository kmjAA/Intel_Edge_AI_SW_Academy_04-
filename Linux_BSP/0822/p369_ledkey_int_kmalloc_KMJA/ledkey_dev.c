...
struct int ledkey_open (struct inode *inode, struct file *filp)
{
   	int i;
	int result;
	int num0 = MAJOR(inode->i_rdev); 
    int num1 = MINOR(inode->i_rdev); 
	char * irqName[GPIOKEYCNT] = {"irqKey0","irqKey1","irqKey2","irqKey3","i    rqKey4","irqKey5","irqKey6","irqKey7"};
	...
	{
		result = request_irq(pKeyData->key_irq[i],key_isr,IRQF_TRIGGER_RISIN    G,irqName[i],pKeyData);
		if(result < 0)
		{
